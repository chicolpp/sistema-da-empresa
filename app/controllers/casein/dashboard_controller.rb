module Casein
	class DashboardController < Casein::CaseinController
		def index
  		...
  		@pocos_manutencao_admin = Poco.select("pocos.id, pocos.cliente_id, pocos.linha_endereco, pocos.apelido_endereco, data_servico, DATE_SUB(NOW(), INTERVAL 3 YEAR), pocos.periodo_manutencao")
  			.joins("LEFT JOIN manutencaos m ON m.poco_id = pocos.id LEFT JOIN manutencao_servicos ms ON ms.manutencao_id = m.id")
  			.joins(:instalacao)
  			.where("pocos.schedule_maintenance_at <= ?", Date.current)
  			.group("pocos.id")
  			.order("ms.data_servico DESC")

  		@pocos_manutencao_admin_data_atual = Poco.select("pocos.id, pocos.cliente_id, pocos.linha_endereco, pocos.apelido_endereco, data_servico, DATE_SUB(NOW(), INTERVAL 3 YEAR), pocos.periodo_manutencao")
  			.joins("LEFT JOIN manutencaos m ON m.poco_id = pocos.id LEFT JOIN manutencao_servicos ms ON ms.manutencao_id = m.id")
  			.joins(:instalacao)
  			.joins("LEFT JOIN manutencao_contatos mc on mc.poco_id = pocos.id ")
  			.where("mc.nova_data = DATE('#{Time.now}')")
  			.where("ms.data_servico < DATE_SUB(NOW(), INTERVAL pocos.periodo_manutencao YEAR)")
  			.group("pocos.id")
  			.order("ms.data_servico DESC")
